...
//	      }
			try {
				...
				InetAddress nodeHostName = InetAddress.getByName(localAddr.toString());
	  ...
	  {
	    System.out.println("Usage: java driver.CAN [options]");
	    System.out.println();
	          ...
	          JistAPI.runAt(new Runnable()
	              {
	                public void run()
	                {
	                  System.out.println("[CAN] "+address+": links="+iarp.getNumLinks()+" routes="+iarp.getNumRoutes());
	                  iarp.showLinks();
	                  iarp.showRoutes();
	                }
